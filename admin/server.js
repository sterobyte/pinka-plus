...
/**
 * Validate initData (Telegram WebApp)
...
function parseInitData(initData) {
  ...
  let user = null;
  if (userRaw) {
    try { user = JSON.parse(userRaw); } catch { return { ok: false, error: "Bad user JSON" }; }
  }

  return { ok: true, user, auth_date: params.get("auth_date") || "" };
}
...
app.post("/api/users/ensure", async (req, res) => {
  ...
  const doc = await User.findOneAndUpdate(
    { tgId },
    {
      $setOnInsert: { tgId, createdAt: new Date(), launchCount: 0 },
      $set: update,
      $inc: { launchCount: 1 }
    },
